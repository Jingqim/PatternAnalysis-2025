# train_unet.py
# UNet trainer with tqdm, early-stop at Dice ≥ 0.90, trains on first 100 samples,
# and cuDNN autotuning enabled.
...
# ---- cuDNN autotune ----
torch.backends.cudnn.benchmark = True        # let cuDNN pick best algos for fixed shapes
torch.backends.cudnn.deterministic = False   # allow fastest (non-deterministic) kernels
...
# -------------------
# Trainer (limit to first N train samples)
# -------------------
def train_unet(
    epochs: int = 10,
    batch_size: int = 8,
    ...
    # Build loaders, then limit training to first N samples
    ...
    # Use same worker/pinning settings, and enable persistent workers + prefetch
    ...
    print(f"Training on {limit} samples | Val: {len(val_ld.dataset)} | Classes: {n_classes} | Device: {device}")
...
if __name__ == "__main__":
    train_unet(
        ...
        target_dice=0.90,
        size_hw=(256,256),
        train_limit=100,      # only first 100 training samples
    )
