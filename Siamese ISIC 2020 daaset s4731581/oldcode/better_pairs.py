...
#%% TRAINING — Full train split; balanced val/test; balanced pair sampler; StepLR decay; tqdm everywhere
...
# --- Balanced on-the-fly pair sampler for TRAIN ---
class OnTheFlyPairsBalanced(torch.utils.data.Dataset):
    """
    Samples pairs with equal probability across three types:
      1) pos-pos (same), 2) neg-neg (same), 3) pos-neg (different).
    Avoids domination by easy neg-neg pairs.
    """
    ...
    def __getitem__(self, _):
        t = int(self.rng.integers(0, 3))  # 0: PP, 1: NN, 2: PN
        if t == 0 and len(self.pos_idx) >= 2:
            i, j = self.rng.choice(self.pos_idx, size=2, replace=False)
            y = 1.0
        elif t == 1 and len(self.neg_idx) >= 2:
            ...
            if len(self.pos_idx) >= 1 and len(self.neg_idx) >= 1:
                ...
                # degenerate fallback if no pos: NN same
                i, j = self.rng.choice(self.neg_idx, size=2, replace=False)
                y = 1.0
...
# --- Balanced pair builder for VAL/TEST (caps pairs for speed) ---
...
def main():
    ...
    # --- Prepare splits (runs ONCE; not in workers) ---
    ...
    # --- Training dataset: ENTIRE train split ---
    train_ds = ISIC2020Dataset(train_df)

    # --- Balanced IMAGE subsets for VAL & TEST (50% pos / 50% neg) ---
    ...
    # --- TRAIN: balanced on-the-fly pair sampling ---
    train_pair_ds = OnTheFlyPairsBalanced(train_ds, length_multiplier=4, seed=123)

    # --- VAL/TEST: balanced PAIR sets, capped for speed ---
    ...
    # --- Model / loss / optim / LR DECAY (StepLR) ---
    model = SiameseNet(EMB_DIM).to(DEVICE)
    criterion = nn.BCELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4)

    # Decay LR by 0.1 every 15 epochs (tweak as you like)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=15, gamma=0.1)

    # --- Train for 50 epochs; validate each epoch ---
    for epoch in range(1, EPOCHS + 1):
        ...
        for x1, x2, y in pbar:
            ...
            optimizer.zero_grad()
            probs = model(x1, x2)              # probability "same"
        ...
        print(f"[epoch {epoch}] train_loss={runL/n:.4f} train_acc={runA/n:.3f} | val_loss={vL:.4f} val_acc={vA:.3f} | lr={optimizer.param_groups[0]['lr']:.2e}")
