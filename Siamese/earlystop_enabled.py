...
def amp_autocast_for(device):
    """Enable autocast only when we're actually using CUDA."""
    if device.type != "cuda":
        class DummyCtx:
...
def train(args):
    ...
    # Model / Opt / Loss
    model = SiameseResNet18().to(device)  # from scratch
    ...
    torch.save({"epoch": epoch if not stopped_early else epoch,
               "model_state": model.state_dict()},
               run_dir / "last.pt")
...
# --------------------- CLI ---------------------
def parse_args():
    p = argparse.ArgumentParser("Siamese ISIC2020 (Windows) — scratch only")
    ...
    p.add_argument("--img-size", type=int, default=256,
                   help="Use 256 (mirror native) or 384.")
    ...
    p.add_argument("--drop-tolerance", type=float, default=0.002,
                   help="Min drop to trigger LR decay.")
    p.add_argument("--lr-decay-factor", type=float, default=0.5,
                   help="Multiply LR by this on drop.")
    p.add_argument("--lr-min", type=float, default=1e-6,
                   help="Lower bound for LR.")
    p.add_argument("--seed", type=int, default=69420)
    return p.parse_args()
