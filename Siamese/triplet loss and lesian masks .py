...
class SiameseResNet18(nn.Module):
    def __init__(self, mask_mode="off", meta_dim=0, mask_l1=0.0):
        ...
        # replace conv1 to accept 4ch if mask used
...
def build_datasets(args, device):
    ...
    if args.use_metadata:
        ...
        meta_train = meta_all[train_df.index]
        meta_val   = meta_all[val_df.index]
        meta_testb = meta_all[test_bal_df.index]
        meta_dim = meta_all.shape[1]
        print(f"[META] enabled. dim={meta_dim}. Using columns like: {meta_names[:6]} ...")
    else:
...
def contrastive_loss_from_embeddings(z1, z2, y, margin=0.5):
    # y=1: same -> minimize distance; y=0: diff -> push apart by margin
    # z are L2-normalized -> Euclidean D = sqrt(2-2cos)
...
def train(args):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if args.loss == "triplet":
            ...
            # BCE/contrastive with pairs (+ optional HNM)
...
# --------------------- CLI ---------------------
def parse_args():
    p = argparse.ArgumentParser("ISIC-2020 Siamese — triplet/contrastive, learned mask, metadata")
